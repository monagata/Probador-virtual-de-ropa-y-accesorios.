    ...
    try:
        ...
        if img_prenda_superior is not None:
            ...
            if color_prenda and color_prenda.strip():
                ...
                prenda_superior_final = img_prenda_superior.copy() if isinstance(img_prenda_superior,
        ...
        if img_prenda_inferior is not None:
            ...
            if color_prenda and color_prenda.strip():
                ...
                prenda_inferior_final = img_prenda_inferior.copy() if isinstance(img_prenda_inferior,
                                                                                 np.ndarray) else np.array(
                    img_prenda_inferior)
        ...
        if prenda_inferior_final is not None and prenda_superior_final is not None:
            ...
            img_con_inferior = gen._aplicar_textura_avanzada(
                img_p_pil, prenda_inferior_final, puntos, depth_norm_temp, tipo_prenda="inferior",
                factor_escala=escala_prenda_inferior
            )
            ...
            mesh = gen.generar(img_con_inferior_pil, prenda_superior_final, puntos, medidas, callback_progreso,
                               tipo_prenda="superior", factor_escala=escala_prenda_superior)

        elif prenda_inferior_final is not None:
            mesh = gen.generar(img_p_pil, prenda_inferior_final, puntos, medidas, callback_progreso,
                               tipo_prenda="inferior", factor_escala=escala_prenda_inferior)

        elif prenda_superior_final is not None:
            mesh = gen.generar(img_p_pil, prenda_superior_final, puntos, medidas, callback_progreso,
                               tipo_prenda="superior", factor_escala=escala_prenda_superior)
...
def ejecutar_evaluacion_completa():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            reporte_lineas.append(
                f"  VRAM en uso:           {vram_usada:.0f} MB ({vram_usada / vram_total * 100:.1f}%)")
        else:
        ...
        for altura, hombros, talla_real in casos_prueba:
            ...
            if prediccion == talla_real:
                aciertos += 1
            reporte_lineas.append(
                f"    {ok} Altura:{altura}cm Hombros:{hombros}cm → Pred:{prediccion} Real:{talla_real}")
        ...
        if (ultima_sesion["img_persona"] is not None and ultima_sesion["puntos"] is not None and
            ...
            if ultima_sesion["img_prenda_superior"] is not None:
                ...
                if "error" not in res_sup:
                    ...
                    reporte_lineas.append(
                        f"  Píxeles dentro del cuerpo:       {res_sup['pixeles_dentro_cuerpo']:,}  ({res_sup['pct_ajuste_correcto']:.1f}%)")
                    reporte_lineas.append(
                        f"  Píxeles FUERA del cuerpo:        {res_sup['pixeles_fuera_cuerpo']:,}  ({res_sup['pct_desajuste']:.1f}%)")
            ...
            if ultima_sesion["img_prenda_inferior"] is not None:
                ...
                if "error" not in res_inf:
                    ...
                    reporte_lineas.append(
                        f"  Píxeles dentro del cuerpo:       {res_inf['pixeles_dentro_cuerpo']:,}  ({res_inf['pct_ajuste_correcto']:.1f}%)")
                    reporte_lineas.append(
                        f"  Píxeles FUERA del cuerpo:        {res_inf['pixeles_fuera_cuerpo']:,}  ({res_inf['pct_desajuste']:.1f}%)")
